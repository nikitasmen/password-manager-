...
bool handleEncryptionSettingsChange(const AppConfig& oldConfig,
    ...
    if (oldEnc != newEnc) {
        ...
        if (!MigrationHelper::getInstance().migrateMasterPasswordForEncryptionChange(
                oldEnc, newEnc, oldTaps, oldInit, newTaps, newInit, masterPassword, dataPath)) {
    ...
    if (lfsrParamsChanged) {
        ...
        if (!MigrationHelper::getInstance().migrateCredentialsForLfsrChange(
                oldTaps, oldInit, newTaps, newInit, masterPassword, dataPath)) {
...
bool MigrationHelper::migrateCredentialsForLfsrChange(const std::vector<int>& oldTaps,
    ...
    try {
        ...
        for (const auto& platform : platforms) {
            ...
            if (credDataOpt) {
                ...
                if (credentials.encryption_type == EncryptionType::LFSR) {
                    ...
                    if (reencryptCredential(
                            platform, credentials, oldEncryptor.get(), newEncryptor.get(), storage.get())) {
                        successCount++;
                    ...
                    successCount++;
                    std::cout << "Skipped non-LFSR credentials for: " << platform << " (no migration needed)"
                              << std::endl;
                }
            } else {
        ...
        bool masterPasswordUpdated =
            updateMasterPasswordWithNewLfsr(oldTaps, oldInitState, newTaps, newInitState, masterPassword, dataPath);
...
bool MigrationHelper::updateMasterPasswordWithNewLfsr(const std::vector<int>& oldTaps,
                                                      const std::vector<int>& oldInitState,
                                                      const std::vector<int>& newTaps,
                                                      const std::vector<int>& newInitState,
                                                      const std::string& masterPassword,
    ...
    return migrateMasterPasswordForEncryptionChange(EncryptionType::LFSR,
                                                    EncryptionType::LFSR,
                                                    oldTaps,
                                                    oldInitState,
                                                    newTaps,
...
bool MigrationHelper::migrateMasterPasswordForEncryptionChange(EncryptionType oldType,
    ...
    std::cout << "Starting master password migration from " << oldTypeStr << " to " << newTypeStr << "..." << std::endl;
    ...
    if (newType == EncryptionType::LFSR) {
        ...
        // For AES and others, use the factory. The salt is not passed directly but will be used to store the final
        // value.
    ...
    std::cout << "Successfully migrated master password from " << oldTypeStr << " to " << newTypeStr << " encryption."
              << std::endl;
    return true;
}

bool MigrationHelper::applySettingsFromConfig(const AppConfig& oldConfig,
                                              const AppConfig& newConfig,
                                              const std::string& masterPassword) {
    if (masterPassword.empty()) {
    ...
    try {
        ...
        if (success) {
            ...
            std::cerr << "Settings migration completed with errors. Some changes may not have been applied."
                      << std::endl;
        }
